...
export const generatePTEReport = async (userId, activityIds) => {
  try {
    ...
    const [userResults] = await connection.query(userSql, [userId]);
    const user = {
    ...
    const [results] = await connection.query(sql, [userId, activityIds]);
    ...
    console.log('generatePTEReport error: ', err);
  }
}
